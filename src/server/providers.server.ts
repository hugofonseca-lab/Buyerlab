...
export class MockSimulationProvider implements SupplierProvider {
  ...
  async reply(
    ...
    const buyerName = stored.config.buyerName?.trim();
    const rng = createRng(stored.run.seed, `saudacao-${stored.estadoPublico.turno}`);
    const prefix =
      buyerName && rng() < 0.35
        ? `${buyerName}, ${prefixBase.charAt(0).toLowerCase()}${prefixBase.slice(1)}`
        : prefixBase;
    const emotion =
      stored.privateState.frustracao > 60
        ...
        : stored.privateState.confianca > 75
          ...
          : "";
    return {
      supplierMessage: [prefix, emotion, info].filter(Boolean).join(" "),
      proposedPrice,
    };
  }
...
function buildReplyPayload(
  ...
  return {
    ...
    buyerName: stored.config.buyerName?.trim() || null,
    authorizedTone:
      stored.privateState.frustracao > 60
        ? "cauteloso e firme"
        : stored.privateState.confianca > 75
