...
export function LegacyFinalOfferPage({ runId }: { runId: string }) {
  const navigate = useNavigate();
  const { snapshot, loading, error, refresh, simulation } = useRun(runId);
  const [offer, setOffer] = useState(defaults);
  useEffect(() => {
    if (snapshot?.suggestedOffer) setOffer(snapshot.suggestedOffer);
  }, [snapshot]);
  ...
  const monthly = useMemo(
    () =>
      offer.precoUnitario *
      (snapshot?.aluminum?.instance.purchaseQuantity ?? orionNexa.mandato.demandaMensal),
    [offer.precoUnitario, snapshot?.aluminum?.instance.purchaseQuantity],
  );
  if (loading)
    return (
      <PageShell>
  ...
  return (
    <PageShell>
      <PageHeader
        eyebrow={`Seed ${snapshot.run.seed}`}
      ...
      <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        {!review ? (
          <>
            <div className="grid gap-6 lg:grid-cols-3">
              <OfferGroup title="Comercial" description="Preço e compromissos básicos.">
                <NumberField
                  label={snapshot.aluminum ? "Preço por tonelada (R$)" : "Preço unitário (R$)"}
                ...
                />
                <NumberField
                  label={snapshot.aluminum ? "Volume mensal (t)" : "Volume mínimo mensal"}
                  value={offer.volumeMinimoMensal}
                  onChange={number("volumeMinimoMensal")}
                  step={snapshot.aluminum ? "1" : "100"}
                  min="0"
                />
                <NumberField
                  label="Duração (meses)"
            ...
            <div className="mt-6 flex flex-col gap-4 border-t pt-6 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs text-muted-foreground">Custo do material proposto</p>
                <p className="font-display text-2xl font-bold">{brl(monthly)}</p>
              </div>
          ...
          <Review
            unit={snapshot.aluminum ? "t/mês" : "un."}
            offer={offer}
            monthly={monthly}
...
function Review({
  ...
  const items = [
    ["Preço unitário", brl(offer.precoUnitario)],
    ["Custo do material (sem custos adicionais)", brl(monthly)],
    ["Posição pública do fornecedor", brl(publicPrice)],
    ["Volume mínimo", `${offer.volumeMinimoMensal.toLocaleString("pt-BR")} ${unit}`],
